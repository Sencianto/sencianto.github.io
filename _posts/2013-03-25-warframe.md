...
* フレームの`ステータス`・`アビリティ`・`シナジー`をPrime版リリース日の昇順に整理
  * `ステータス`
    * Prime版の全フレームのランク30時のステータス平均値と比較
...
{% comment %} Prime版の全Warframeランク30時のステータス平均値 {% endcomment %}
...
| No | 評価 |  | 補足 |
| :-----: | :-----: | :----- | :----- |
| 0 | ○ | Swordsmanship | 効果は低いが影響範囲が広く、堅実なバフ |
| 1 | ✕ | Slash Dash | ダメージ不足 |
...
| | ✕ | Radiant Finish | フィニッシャー攻撃のみ有効、効率が悪い |
| 3 | ✕ | Radial Javelin | 基礎ダメージが低い、近接攻撃扱いではない |
| | ✕ | → Radial Javelin | 低レベルのミッション向け、広範囲低ダメージ |
